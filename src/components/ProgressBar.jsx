  ...
  return (
    <div className="progress-container" style={{ display: 'flex', gap: '6px', background: 'transparent', height: '40px', alignItems: 'center', overflow: 'visible' }}>
      {Array.from({ length: totalSlides }).map((_, index) => {
        ...
        // Lấy tiêu đề slide để hiển thị tooltip (ưu tiên subTitle nếu có)
        ...
        return (
          <div 
            ...
            style={{ 
              flex: 1, 
              height: '6px', 
              background: isActive ? 'var(--msb-orange)' : (isPast ? '#cbd5e0' : '#e9ecef'),
              borderRadius: '4px',
              cursor: 'pointer',
              position: 'relative',
          ...
          >
            {/* Hiệu ứng hover nhẹ cho các slide không active */}
            {hoveredIndex === index && !isActive && (
               <div style={{position: 'absolute', inset: 0, background: 'rgba(240, 110, 29, 0.3)', borderRadius: '4px'}}></div>
            )}

            {/* Tooltip hiển thị tiêu đề */}
            <AnimatePresence>
              {hoveredIndex === index && (
                <motion.div
